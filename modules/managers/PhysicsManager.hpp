        ...
        if ((obj0 == vehicle->getRigidBody() && isCoin(obj1)) || (obj1 == vehicle->getRigidBody() && isCoin(obj0))) {
            ...
            if (isNextCheckpoint(obj0 == vehicle->getRigidBody() ? obj1 : obj0)) {
                ...
                if (nextCheckpointIndex >= checkpointsLap.size()) {
                    ...
                    changeCircuitSubject.notifyChangeCircuit();
...
struct PhysicsManager : public Manager, public Observer {
...
protected:
    ...
    void onChangeCircuit() override {
        ...
        if (firstLap && !secondLap) {
            ...
            json scJson = *sceneJson;
            auto instanceIt = std::find_if(scJson["instances"].begin(), scJson["instances"].end(), [&targetId](const json& instance) {
                return instance["id"] == "dir_barrier_inner";
            });
            auto modelIt = std::find_if(scJson["models"].begin(), scJson["models"].end(), [](const json& model) {
                return model["id"] == "dir_barrier_inner";
            });
            
            if (modelIt != scJson["models"].end()) {
