...
*/
/*
vec3 OptimizedBRDF(vec3 Albedo, vec3 Norm, vec3 EyeDir, vec3 LD) {
    vec3 Diffuse;
    vec3 Specular;
    
    // Calcola la componente Diffuse
    float NdotL = max(dot(Norm, LD), 0.0f);
    Diffuse = Albedo * NdotL;
    
    // Calcola la componente Specular
    vec3 ReflectDir = reflect(-LD, Norm);
    float SpecFactor = max(dot(EyeDir, ReflectDir), 0.0f);

    // Usa una funzione di approssimazione per la potenza
    float SpecularStrength = SpecFactor * SpecFactor; // Approssimazione per esponente 2
    SpecularStrength *= SpecularStrength; // Approssimazione per esponente 4
    SpecularStrength *= SpecularStrength; // Approssimazione per esponente 8
    SpecularStrength *= SpecularStrength; // Approssimazione per esponente 16
    SpecularStrength *= SpecularStrength; // Approssimazione per esponente 32
    SpecularStrength *= SpecularStrength; // Approssimazione per esponente 64
    SpecularStrength *= SpecularStrength; // Approssimazione per esponente 128
    SpecularStrength = sqrt(SpecularStrength); // Approssimazione per esponente 160
    
    Specular = vec3(SpecularStrength);
    
    // Ritorna la somma delle componenti Diffuse e Specular
    return Diffuse + Specular;
}*/
